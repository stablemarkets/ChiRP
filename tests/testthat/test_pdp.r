...
test_that("Test Iteration Errors", {
  ...
  d <- data.frame(y = sample(c(0,1), n, T), x = rnorm(n))
  d$y[1:30] <- 0
...
test_that("Test d_train inputs", {
  set.seed(1)
  n<-200 ## generate from clustered, skewed, data distribution
  d <- data.frame(y = sample(c(0,1), n, T), x = rnorm(n), z=sample(c(1,2), size = n, replace = T))
...
test_that("Test d_test inputs", {
  ...
  d <- data.frame(y = sample(c(0,1), n, T), x = rnorm(n), z=sample(c(1,0), size = n, replace = T))
  n<-50 ## generate from clustered, skewed, data distribution
  dt <- data.frame(y = sample(c(0,1), n, T), x = rnorm(n), z=sample(c(1,0), size = n, replace = T))
...
test_that("Test PDP outputs", {
  ...
  dt <- data.frame(y = sample(c(0,1), n, T), x = rnorm(n))

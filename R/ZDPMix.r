...
#' @param gamma_prior_mean Optional. If there are `p` covariates as length `p+1` `as.numeric` vector specifying mean of the Gaussian prior on the zero probability logistic model's conditional mean parameter vector. Default is a vector of zeros - i.e., null-centered prior mean.
#' @param gamma_prior_var Optional. If there are `p` covariates as length `p+1` `as.numeric` vector specifying variance of the Gaussian prior on the zero probability logistic model's conditional mean parameter vector. Default is vector of twos - moderately flat on the odds ratio scale.
